<template>
	<li class="nav-item nav-search">
		...
		<div class="search-input" :class="{ open: showSearchBar }">
			...
			<!-- <input type="text" placeholder="Explore SJU...." class="form-control-input"> -->
			<!-- @keyup.esc="escPressed" -->
			<!-- @keyup.enter="suggestionSelected" -->
			<b-form-input
				v-if="showSearchBar"
				v-model="searchQuery"
				:placeholder="$t('Explore SJU')"
				autofocus
				autocomplete="off"
			...
			<!-- Suggestions List -->
			<vue-perfect-scrollbar :settings="perfectScrollbarSettings" class="search-list search-list-main scroll-area overflow-hidden" :class="{ show: searchQuery }" tagname="ul">
				<li v-for="(suggestion_list, grp_name, grp_index) in filteredData" :key="grp_index" class="suggestions-groups-list">
					...
					<ul>
						<li v-for="(suggestion, index) in suggestion_list" :key="index" class="suggestion-group-suggestion cursor-pointer" :class="{ 'suggestion-current-selected': currentSelected === `${grp_index}.${index}` }" @mouseenter="currentSelected = `${grp_index}.${index}`" @mousedown.prevent="suggestionSelected(grp_name, suggestion)">
