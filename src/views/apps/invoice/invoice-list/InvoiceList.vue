<template>
	...
	<b-card no-body title="User Timeline">
		<div class="m-2">
			...
			<b-row>
				...
				<b-col cols="12" md="6" class="d-flex align-items-center justify-content-start mb-1 mb-md-0">
					...
					<b-button variant="primary" :to="{ name: 'apps-invoice-add' }"> Add Record </b-button>
				</b-col>
		...
		<b-table ref="refInvoiceListTable" :items="fetchInvoices" responsive :fields="tableColumns" primary-key="id" :sort-by.sync="sortBy" show-empty empty-text="No matching records found" :sort-desc.sync="isSortDirDesc" class="position-relative">
			...
			<template #cell(client)="data">
				<b-media vertical-align="center">
					<template #aside>
						<b-avatar size="32" :src="data.item.avatar" :text="avatarText(data.item.client.name)" :variant="`light-${resolveClientAvatarVariant(data.item.invoiceStatus)}`" />
					</template>
			...
			<template #cell(actions)="data">
				<div class="text-nowrap">
					...
					<feather-icon :id="`invoice-row-${data.item.id}-preview-icon`" icon="EyeIcon" size="16" class="mx-1" @click="$router.push({ name: 'apps-invoice-preview', params: { id: data.item.id } })" />
