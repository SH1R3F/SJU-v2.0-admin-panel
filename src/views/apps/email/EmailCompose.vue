<template>
	<b-modal id="compose-mail" :visible="shallShowEmailComposeModal" title="Compose Email" modal-class="modal-sticky" footer-class="d-flex justify-content-between" body-class="p-0" size="lg" no-fade hide-backdrop static @change="(val) => $emit('update:shall-show-email-compose-modal', val)">
		...
		<template #modal-footer>
			<!-- Footer: Left Content -->
			<div>
				<b-dropdown v-ripple.400="'rgba(255, 255, 255, 0.15)'" split text="Send" variant="primary" right @click="sendEmail">
			...
			<div>
				...
				<b-dropdown variant="link" no-caret toggle-class="p-0" right>
					...
					<b-dropdown-item> Add Label </b-dropdown-item>

					<b-dropdown-item> Plain Text Mode </b-dropdown-item>

					<b-dropdown-divider />

					<b-dropdown-item> Print </b-dropdown-item>
					<b-dropdown-item> Check Spelling </b-dropdown-item>
				</b-dropdown>
		...
		<b-form>
			...
			<div class="compose-mail-form-field">
				...
				<v-select v-model="composeData.to" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" multiple label="name" class="flex-grow-1 email-to-selector" :close-on-select="false" :options="emailToOptions" input-id="email-to">
			...
			<div v-show="showCcField" class="compose-mail-form-field">
				...
				<v-select v-model="composeData.cc" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" multiple label="name" class="flex-grow-1 email-cc-selector" :close-on-select="false" :options="emailToOptions" input-id="email-cc">
					<template #option="{ avatar, name }">
						<b-avatar size="sm" :src="avatar" />
						<span class="ml-50"> {{ name }}</span>
					</template>
			...
			<div v-show="showBccField" class="compose-mail-form-field">
				...
				<v-select v-model="composeData.bcc" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" multiple label="name" class="flex-grow-1 email-bcc-selector" :close-on-select="false" :options="emailToOptions" input-id="email-bcc">
					<template #option="{ avatar, name }">
						<b-avatar size="sm" :src="avatar" />
						<span class="ml-50"> {{ name }}</span>
					</template>
