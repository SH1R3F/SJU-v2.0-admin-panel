	...
	<div>
		...
		<validation-observer #default="{ handleSubmit }" ref="refFormObserver">
			<b-form @submit.prevent="handleSubmit(onSubmit)" @reset.prevent="resetForm">
				...
				<b-row>
					<b-col cols="12">
						<validation-provider #default="validationContext" vid="map_link" :name="$t('Map link')">
							...
							<b-form-group :label="$t('Photos')" label-for="photos">
								...
								<b-alert variant="secondary" show v-if="formData.images.length">
									<div class="alert-body">
										<b-img
											v-for="(photo, i) in formData.images"
											:key="i"
											thumbnail
											:src="photo"
											style="height: 150px; object-fit: cover; margin-inline-end: 7px"
											@click="removePhoto(i)"
										/>
									</div>
								</b-alert>
				...
				<b-row>
					<b-col cols="12">
						<validation-provider
							#default="validationContext"
							vid="questionnaire_id"
							:name="$t('Questionnaire')"
						>
							<b-form-group :label="$t('Questionnaire')" label-for="questionnaire_id">
								<v-select
									v-model="formData.questionnaire_id"
									:dir="$store.state.appConfig.isRTL ? 'ltr' : 'rtl'"
									:options="namings.questionnaires"
									label="name_ar"
									:reduce="(questionnaire) => questionnaire.id"
									:clearable="true"
									:state="getValidationState(validationContext)"
								/>
								<b-form-invalid-feedback>
...
<script>
	...
	export default {
		...
		setup(props) {
			...
			if (!store.hasModule(COURSE_NAMING_STORE_MODULE_NAME))
				store.registerModule(COURSE_NAMING_STORE_MODULE_NAME, namingStoreModule)
			onUnmounted(() => {
				if (store.hasModule(COURSE_NAMING_STORE_MODULE_NAME))
					store.unregisterModule(COURSE_NAMING_STORE_MODULE_NAME)
			})
