	...
	<div>
		...
		<validation-observer #default="{ handleSubmit }" ref="refFormObserver">
			<b-form @submit.prevent="handleSubmit(onSubmit)" @reset.prevent="resetForm">
				...
				<b-row>
					<b-col cols="12">
						<validation-provider #default="validationContext" vid="name" :name="`${$t('Name')} (${$t('In English')})`" rules="min:3">
							<b-form-group :label="`${$t('Name')} (${$t('In English')})`" label-for="name_en">
								<b-form-input id="name_en" v-model="formData.name_en" autofocus :state="getValidationState(validationContext)" trim />
								<b-form-invalid-feedback>
									{{ validationContext.errors[0] }}
								</b-form-invalid-feedback>
							</b-form-group>
	...
	export default {
		...
		setup(props) {
			...
			const onSubmit = function () {
				store
					.dispatch("course-questionnaire/updateQuestionnaire", { id: router.currentRoute.params.id, form: formData.value })
					.then((response) => {
