		...
		<b-card :title="$t('Enrolled courses')" class="mb-0">
			...
			>
					...
					<b-media vertical-align="center" class="align-items-center">
						<template #aside>
							<b-avatar
								...
								:text="avatarText(dblocalize(data.item, 'name').split(' ')[0])"
				...
				<template #cell(attendance)="data">
					<b-badge
						pill
						:variant="data.item.pivot.attendance == 1 ? 'light-primary' : 'light-danger'"
	...
	export default {
		...
		setup() {
			...
			const getCertificate = (event) => {
				const url = `${process.env.VUE_APP_API_BASEURL}/courses/${event}/certificate/volunteer/${router.currentRoute.params.id}`
				axios
					.get(url)
					.then((response) => {
