<template>
	<div>
		<volunteers-list-add-new :is-add-new-volunteer-sidebar-active.sync="isAddNewVolunteerSidebarActive" @refetch-data="refetchData" />

		<!-- Filters -->
		<volunteers-list-filters :name-filter.sync="nameFilter" :mobile-filter.sync="mobileFilter" :email-filter.sync="emailFilter" />
		...
		<b-card no-body class="mb-0">
			...
			<b-table ref="refVolunteerListTable" class="position-relative" :fields="tableColumns" :items="fetchVolunteers" responsive primary-key="id" :sort-by.sync="sortBy" show-empty :empty-text="$t('No matching records found')" :sort-desc.sync="isSortDirDesc">
				<!-- Column: # -->
				...
				<template #cell(volunteer)="data">
					<b-media vertical-align="center" class="align-items-center">
						<template #aside>
							<b-avatar size="32" :src="data.item.avatar" :text="avatarText(data.item.fullName.split(' ')[0])" :variant="data.item.avatar ? '' : 'light-success'" :to="{ name: 'show-volunteer', params: { id: data.item.id } }" />
						</template>
						<b-link :to="{ name: 'show-volunteer', params: { id: data.item.id } }" class="font-weight-bold d-block text-nowrap">
							{{ data.item.fullName }}
						</b-link>
					</b-media>
				</template>
			...
			<div class="mx-2 mb-2">
				<b-row>
					...
					<b-col cols="12" sm="6" class="d-flex align-items-center justify-content-center justify-content-sm-end">
						<b-pagination v-model="currentPage" :total-rows="totalVolunteers" :per-page="perPage" first-number last-number class="mb-0 mt-1 mt-sm-0" prev-class="prev-item" next-class="next-item">
							<template #prev-text>
