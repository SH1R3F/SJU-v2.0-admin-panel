<template>
	<div>
		...
		<b-media class="mb-2">
			<template #aside>
				<b-avatar ref="previewEl" :src="formData.avatar" :text="avatarText(dblocalize(volunteerData, 'fname'))" :variant="formData.avatar ? '' : `light-success`" size="90px" rounded />
			</template>
			<h4 class="mb-1">
					...
					<b-col cols="12" md="12">
						<b-form-group :label="$t('Email')" label-for="email">
							<validation-provider #default="validationContext" name="email" rules="required|email">
								<b-form-input id="email" v-model="formData.volunteerEmail" autofocus :state="getValidationState(validationContext)" trim />
