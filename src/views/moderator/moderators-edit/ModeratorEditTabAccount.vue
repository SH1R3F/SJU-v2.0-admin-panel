<template>
	<div>
		...
		<b-media class="mb-2">
			<template #aside>
				<b-avatar ref="previewEl" :src="formData.avatar" :text="avatarText(moderatorData.username)" :variant="formData.avatar ? '' : `light-success`" size="90px" rounded />
			</template>
			<h4 class="mb-1">
					...
					<b-col cols="12" md="6">
						<b-form-group :label="$t('Username')" label-for="username">
							<validation-provider #default="validationContext" name="username" rules="required">
								<b-form-input id="username" v-model="formData.username" autofocus :state="getValidationState(validationContext)" trim />
					...
					<b-col cols="12" md="12">
						<b-form-group :label="$t('Email')" label-for="email">
							<validation-provider #default="validationContext" name="email" rules="required|email">
								<b-form-input id="email" v-model="formData.adminEmail" :state="getValidationState(validationContext)" trim />
								<b-form-invalid-feedback>
									{{ validationContext.errors[0] }}
								</b-form-invalid-feedback>
							</validation-provider>
					...
					<b-col cols="12" md="12">
						<b-form-group :label="$t('Role')" label-for="role">
							<validation-provider #default="validationContext" vid="role" :name="$t('Role')" rules="required">
								<v-select v-model="formData.role_id" :dir="$store.state.appConfig.isRTL ? 'ltr' : 'rtl'" :options="roles" label="display_name" :reduce="(role) => role.id" />
...
<script>
	...
	export default {
		...
		setup(props) {
			...
			const { avatar, email: adminEmail, username, mobile, role_id } = props.moderatorData;
			const oldData = { avatar, adminEmail, username, mobile, role_id };
			...
			const onSubmit = function () {
				store
					.dispatch("app-moderator/updateModerator", { id: router.currentRoute.params.id, form: formData.value })
					.then((response) => {
