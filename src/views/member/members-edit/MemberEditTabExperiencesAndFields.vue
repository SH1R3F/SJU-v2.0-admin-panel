<template>
	<b-form class="repeater-form" @submit.prevent="onSubmit">
		...
		<b-row v-for="(experience, index) in experiences" ref="experienceRow" :key="`exp-${index}`">
			...
			<b-col lg="2" md="3" class="mb-50">
				<b-button
					...
					class="mt-0 mt-md-2"
					@click="removeItem(index, 'experience')"
				>
		...
		<b-button
			...
			:disabled="Object.keys(experiences[experiences.length - 1] || {}).length != 2 && experiences.length !== 0"
		>
		...
		<b-button
			...
			variant="primary"
			@click="addItem('field')"
			:disabled="Object.keys(fields[fields.length - 1] || {}).length != 1 && fields.length !== 0"
		>
			<feather-icon icon="PlusIcon" class="mr-25" />
			<span>{{ $t("Add New") }}</span>
		</b-button>
		...
		<b-row v-for="(language, index3) in languages" ref="languageRow" :key="`lang-${index3}`">
			...
			<b-col lg="2" md="3" class="mb-50">
				<b-button
					...
					variant="outline-danger"
					class="mt-0 mt-md-2"
					@click="removeItem(index3, 'language')"
				>
					<feather-icon icon="XIcon" class="mr-25" />
					<span>{{ $t("Delete") }}</span>
				</b-button>
		...
		<b-button
			v-ripple.400="'rgba(255, 255, 255, 0.15)'"
			variant="primary"
			@click="addItem('language')"
			:disabled="Object.keys(languages[languages.length - 1] || {}).length != 2 && languages.length !== 0"
		>
			<feather-icon icon="PlusIcon" class="mr-25" />
			<span>{{ $t("Add New") }}</span>
		</b-button>
	...
	export default {
		...
		setup(props) {
			...
			const {
				...
				fields: flds = [],
				languages: langs = [],
			} = props.memberData.experiences_and_fields || []
