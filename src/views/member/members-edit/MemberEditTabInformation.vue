		...
		<validation-observer #default="{ handleSubmit }" ref="refFormObserver">
			<b-form @submit.prevent="handleSubmit(onSubmit)" @reset.prevent="resetForm">
					...
					<b-col cols="12" lg="3">
						<b-form-group :label="$t('Birth date') + ' (' + $t('Meladi') + ')'">
							<validation-provider
								#default="validationContext"
								vid="birthday_meladi"
								:name="$t('Birth date') + ' (' + $t('Meladi') + ')'"
								rules="required"
							>
								<date-picker
									v-model="formData.birthday_meladi"
									:locale="$i18n.locale == 'ar' ? 'ar-sa' : ''"
									:state="getValidationState(validationContext)"
								></date-picker>
								<b-form-invalid-feedback>
					...
					<b-col cols="12" lg="3">
						<b-form-group :label="`${$t('Birth date')} (${$t('Hijri')})`">
							<validation-provider
								#default="validationContext"
								vid="birthday_hijri"
								:name="$t('Birth date') + ' (' + $t('Hijri') + ')'"
								rules="required"
							>
								<date-picker
									v-model="formData.birthday_hijri"
									:locale="$i18n.locale == 'ar' ? 'ar-sa' : ''"
									calendar="hijri"
									:state="getValidationState(validationContext)"
								></date-picker>
				...
				<b-row>
					...
					<b-col cols="12" md="6">
						<b-form-group
							:label="`${$t('Job title')} (${$t('Journalist')})`"
							label-for="journalist_job_title"
						>
							<validation-provider
								#default="validationContext"
								vid="journalist_job_title"
								:name="`${$t('Job title')} (${$t('Journalist')})`"
								rules="required|min:3"
							>
								<b-form-input
									id="journalist_job_title"
									v-model="formData.journalist_job_title"
									:state="getValidationState(validationContext)"
									trim
								/>
								<b-form-invalid-feedback>
					...
					<b-col cols="12" md="6">
						<b-form-group
							:label="`${$t('Employer')} (${$t('Journalist')})`"
							label-for="journalist_employer"
						>
							<validation-provider
								#default="validationContext"
								vid="journalist_employer"
								:name="`${$t('Employer')} (${$t('Journalist')})`"
								rules="required|min:3"
							>
								<b-form-input
									id="journalist_employer"
									v-model="formData.journalist_employer"
									:state="getValidationState(validationContext)"
									trim
								/>
								<b-form-invalid-feedback>
					...
					<b-col cols="12">
						<b-form-group :label="$t('Newspaper type')" label-for="newspaper_type">
							<validation-provider
								#default="validationContext"
								vid="newspaper_type"
								:name="$t('Newspaper type')"
								rules="required"
							>
								<v-select
									v-model="formData.newspaper_type"
									:dir="$store.state.appConfig.isRTL ? 'ltr' : 'rtl'"
									:options="$newspaperTypes"
									:reduce="(type) => type.value"
									input-id="newspaper_type"
									:state="getValidationState(validationContext)"
								/>
								<b-form-invalid-feedback>
									{{ validationContext.errors[0] }}
					...
					<b-col cols="12" md="6">
						<b-form-group :label="`${$t('Job title')} (${$t('Non-journalist')})`" label-for="job-title">
							<validation-provider
								#default="validationContext"
								vid="job_title"
								:name="`${$t('Job title')} (${$t('Non-journalist')})`"
								rules="required|min:3"
							>
								<b-form-input
									id="job_title"
					...
					<b-col cols="12" md="6">
						<b-form-group :label="`${$t('Employer')} (${$t('Non-journalist')})`" label-for="employer">
							<validation-provider
								#default="validationContext"
								vid="employer"
								:name="`${$t('Employer')} (${$t('Non-journalist')})`"
								rules="required|min:3"
							>
								<b-form-input
									id="employer"
...
<script>
	...
	import { $genders, $newspaperTypes, $countries, $nationalities } from "@siteConfig"
